...
namespace ProjectX_API_Activity2.Controllers
{
    public class CourseController : ApiController
    {
        ...
        [HttpPost]
        [ActionName("AddCoures")]
        public HttpResponseMessage AddCoures(CourseDTO inputCourseObj)
        {
            try
            {
              
                if (inputCourseObj != null && inputCourseObj.CourseTitle != null && inputCourseObj.AssessmentMode != null && inputCourseObj.CourseSyllabus != null && inputCourseObj.CourseTitle != null && && inputCourseObj.HoursAssigned != null)
                {
                    blObj = new ProjectXBL();
                    int retVal = blObj.AddNewCourse(inputCourseObj);
                    if (retVal == 1)
                    {
